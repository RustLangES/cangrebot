    ...
    for meetup in meetups {
        let meetup_future = async move {
            if let Ok(meetup_fields) = get_meetup(meetup.clone()).await {
                ...
                let Ok(_) = msg.channel_id.say(&ctx.http, format!("No hay eventos próximos para {}", meetup)).await else {
        ...
        let detail_future = async move {
            get_detail(link).await.expect("No se pudo obtener el detalle del evento")
        };
...
async fn get_meetup(group:String) -> Result<(String, String, String), Box<dyn std::error::Error  + Send + Sync>>  {
    ...
    let event_title_selector = scraper::Selector::parse("#event-card-e-1 span.ds-font-title-3").unwrap();
...
async fn get_detail(link: String) -> Result<(String, String, String), Box<dyn std::error::Error>>  {
    ...
    let image = document2.select(&image_selector).next().unwrap().value().clone();
    let image = image.attr("content").unwrap();

    let description = document2.select(&scraper::Selector::parse("#event-details > div.break-words").unwrap()).next().unwrap();
    let description = description.text().collect::<Vec<_>>().join(" ");
    let mut description = description[0..280].to_string();
    description.push('…');
    
    let location_div = document2.select(&scraper::Selector::parse("#event-info div.overflow-hidden div").unwrap()).next().unwrap();
