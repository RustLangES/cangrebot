...
impl TtsNormalizer {
    ...
    fn normalize_caps(text: &str) -> String {
        static RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"\b[A-ZÁÉÍÓÚÑ]{4,}\b").unwrap());

        RE.replace_all(text, |caps: &Captures| Cow::Owned(caps[0].to_lowercase()))
            .into_owned()
    }
    ...
    fn normalize_paths(text: &str) -> String {
        static RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?:^|\s)(/[\w\-./]+)").unwrap());
   ...
   fn normalize_laughter(text: &str) -> String {
        static LAUGH_RE: LazyLock<Regex> = LazyLock::new(|| {
            Regex::new(r"(?i)(ja|ha|je|he|ji|hi|ho|aj|ah|eh|ih|oh|uh|xs|ks|hue){3,}").unwrap()
        });

        static REPEAT_CHAR: LazyLock<Regex> =
            LazyLock::new(|| Regex::new(r"(?i)([a-záéíóúñ])\1{4,}").unwrap());

        static REPEAT_WORD: LazyLock<Regex> = LazyLock::new(|| {
            Regex::new(r"(?i)\b(\w{1,4})\s+\1\s+\1(\s+\1)*\b").unwrap()
        });

        static REPEAT_PUNCT: LazyLock<Regex> =
            LazyLock::new(|| Regex::new(r"([!?.]){4,}").unwrap());

        let mut result = text.to_string();

        result = LAUGH_RE.replace_all(&result, "risa").into_owned();

        result = REPEAT_CHAR
            .replace_all(&result, |caps: &Captures| {
                let char = &caps[1];
                let repeat_count = caps[0].len();
                if repeat_count >= 5 {
                    Cow::Owned(char.repeat(2))
                } else {
                    Cow::Borrowed(&caps[0])
                }
            })
        ...
        result = REPEAT_WORD
            .replace_all(&result, |caps: &Captures| {
                ...
                let count = full_match.matches(word).count();

                if count >= 3 {
                    Cow::Owned(format!("{} repetido", word))
                } else {
                    Cow::Borrowed(full_match)
                }
            })
            .into_owned();
        ...
        result = REPEAT_PUNCT
            .replace_all(&result, |caps: &Captures| {
                let punct = &caps[1];
                Cow::Owned(punct.repeat(2))
            })
            .into_owned();

        Self::check_excessive_repetition(&result)
    }
